...
copyimage() { # A systemimage.tar.gz file can be used: `setupTermuxArch.sh ./[path/]systemimage.tar.gz` and `setupTermuxArch.sh /absolutepath/systemimage.tar.gz`
	cfile="${1##/*/}" 
 	file="$cfile" 
	if [[ "$lc" = "" ]];then
		cp "${wdir}$1".md5  "$installdir" & echo "Copying $1…" 
		cp "${wdir}$1" "$installdir" & echo "Copying $1…"
	elif [[ "$lc" = "1" ]];then
		cp "$idir/$cfile".md5  "$installdir" & echo "Copying $1…"
		cp "$idir/$cfile" "$installdir" & echo "Copying $1…"
	fi
}
